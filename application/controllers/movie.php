...
* 
*/
class Movie extends Front_Controller
{
	function __construct()
	{
		parent::__construct();
		$this->load->model('front/movie_m');
		$this->load->helper('file');
		$this->table = 'movie_torrent';
	}
	function index(){
		//
		$table = $this->table;
		...
		$url = 'index.php/movie?';
		$total = $this->movie_m->posts_count($table,$cate);
		$posts = $this->movie_m->category_post($cate,$table,$num,$start);
		...
		$view_arr = array('front/head','movie/list','front/footer');
	...
	function id($id){
		...
		$comment = $this->movie_m->movie_comments($table, $cond, 'time DESC', $num=100, $start=0);
		//
		$table = 'user';
		...
		$view_arr = array('front/head-single','movie/single','front/footer');
		$this->load_views($view_arr,$data);
	}
	//
	...
	// callback function for comment process method
	...
	function add_comment(){
		...
		$data = $temp;
		$result = $this->movie_m->insert($table,$data);
		if($result==1){
			echo 'true';
		}else{
			echo 'false';
	...
	function rating(){
		...
		$status = $this->movie_m->rating($table, $cond, $field);
		if ($status==1) {
			echo "true";
		}else{
			echo "false";
