...
class Book extends Front_Controller
{
	...
	function index(){
		...
		$url = 'index.php/book?';
		$total = $this->book_m->posts_count($table,$cate);
		$posts = $this->book_m->category_post($cate,$table,$num,$start);
		...
		$view_arr = array('front/head','book/list','front/footer');
	...
	function id($id){
		...
		$comment = $this->book_m->book_comments($table, $cond, 'time DESC', $num=100, $start=0);
		//
		$table = 'user';
		...
		$view_arr = array('front/head-single','book/single','front/footer');
		$this->load_views($view_arr,$data);
	}
	// formating comment
	...
	function add_comment(){
		...
		$data = $temp;
		$result = $this->book_m->insert($table,$data);
		if($result==1){
			echo 'true';
		}else{
			echo 'false';
	...
	function rating(){
		...
		$status = $this->book_m->rating($table, $cond, $field);
		if ($status==1) {
			echo "true";
		}else{
			echo "false";
