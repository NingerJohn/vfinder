...
<?php //ksort($posts['preview_info']); print_r($posts['preview_info']); ?>
<?php //print_r($session); ?>
<!-- </pre>  -->
...
<div class="container tv">
	<a name="rating-box"></a>
	<!-- Needs to be modified -->
	<h3 style="color:rgb(255, 154, 0)"><?php echo $tv?></h3>
	<hr class="tv-title-hr">
	<!-- Needs to be modified -->
	<div class="container tv-info">
		...
		<div class="sub-info-row title">
			<!-- Needs to be modified -->
			<?php echo $tv_name.$colon.$info['title']; ?>
		</div>
		...
		<div class="sub-info-row download-link">
			<!-- Needs to be modified -->
			<a href="<?php echo site_url('tv/download_torrent').'/'.$info['torrent_id']?>"class="btn btn-primary download"><?php echo $download; ?></a>
		</div>
...
<script type="text/javascript">
	...
	$('input.add-comment-btn').click(function(){
		...
		var type = 'post';
		//Needs to be modified
		var url = '<?php echo site_url("tv/login_check") ?>';
		...
		if(status=='true'){
			var type = "POST";
			//Needs to be modified
			var url = "<?php echo site_url('tv/add_comment/') ?>";
	...
	$('a.up-btn').click(function(){
		...
		if (result === "true") {
			...
			//alert(rate);
			var type = 'POST';
			var url = '<?php echo site_url("tv/rating"); ?>';
