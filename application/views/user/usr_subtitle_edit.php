<div class="container upload-window">
	...
	<!--  -->
	<div class="shortcut">
		<a href="<?=base_url('index.php/user?manage=subtitle&act=go_back')?>" class="btn btn-info"><?php echo $go_back?></a>
	</div>
	<hr/>
	<!-- warning -->
	...
	<div class="container upload-tips">
		<font style="color:green"><?php echo $sub_upload_tips ?></font>
	</div>
	<!--  -->
	<hr>
	<!--  -->
	<div class="container upload-box">
		...
		<div class="upload-dialog">
			...
			<form action="<?=base_url().'index.php/user/upload_subtitle'?>" target="hidden_frame" method="post" enctype="multipart/form-data">
	...
	<div class="container submit">
		<form action="<?php echo base_url("index.php/user?manage=subtitle")?>" method="post" class="submit">
...
/*	$('button.submit').click(function(){
	...
	var cateData = '{"movie":"<?php echo $movie?>","tv":"<?php echo $tv?>","music":"<?php echo $music?>"}';
	var temp = $.parseJSON(cateData);
	initialize(temp);
	// output object data
	function echoArr(arr){
		var result = '';
		$.each(arr,function(oneKey,oneVal){
			result += (oneKey+': '+oneVal+'<br>');
		})
		return result;
	}
	...
	function showResult(data){
		...
		$('input.filesize').val(data['file_size']); // set the value of subtitle file's size
	...
	function deleteFile(){
		...
		type = "post";
		url = '<?=base_url()."index.php/user/delete_temp_subtitle"?>';
		//alert(data);
		//return false;
	...
	$('input.submit-btn').click(function(){
		...
		//alert(temp);
		//return false;
		var url = '<?php echo base_url("index.php/user/subtitle_submit/")?>';
		var type = 'post';
		var data = temp;
		var success = function(msg){
			alert("<?php echo $upload_success;?>");
			location.href='<?php echo base_url("index.php/user?manage=subtitle")?>';
		};
